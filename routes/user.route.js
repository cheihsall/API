...
// Login
userExpressRoute.route('/connect').post(async (req, res, next) => { //'asunc' for make this midleware asynchrone
    ...
    if (!existingUser) {
        ...
        return res.status(200).json({ message: "oups! cet email est incorrect", code: "noEmail" });
        //return res.status(404).send("email doesn't exist...!");
    } else if (existingUser.etat == false) {

        return res.status(200).json({ message: "ce compte est desactivé...!", code: "compteNoActive" });
    ...
    if (!isPasswordValid) {
        ...
        return res.status(200).json({ message: "mot de passe incorrect", code: "noPassword" });
...
userExpressRoute.route('/').get(security,(req, res) => {
...
userExpressRoute.route('/user/:id').get(security,(req, res) => {
...
userExpressRoute.route('/deleteUser/:id').delete(security,(req, res) => {
...
userExpressRoute.route('/updateUser/:id').put(security,(req, res) => {
